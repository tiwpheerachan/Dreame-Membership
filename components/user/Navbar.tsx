...
import { Home, Package, Ticket, UserRound, Coins, Gift, Sparkles } from 'lucide-react'
...
const NAV = [
  ...
  { href: '/privileges', icon: Sparkles,   label: 'สิทธิ' },
...
export default function Navbar() {
  ...
  return (
    <nav style={{
      ...
      {/* SVG filter that powers the glass distortion (mounted once, hidden) */}
      <GlassFilter />
      ...
      <div style={{
        ...
        <GlassEffect radius={9999} className="w-full">
          <div style={{
            position: 'relative',
            display: 'flex',
            padding: 5,
              ...
              return (
                <Link
                  ...
                  style={{
                    ...
                    textDecoration: 'none',
                    // Gold gradient when active; transparent when not (lets the glass show through)
                    background: active
                      ? 'linear-gradient(180deg, #FAF3DC 0%, #EADBB1 35%, #C9A063 75%, #A0782B 100%)'
                      : 'transparent',
                    color: active ? '#1A1815' : 'rgba(40,38,32,0.65)',
                    // Active pill gets a 3D-coined effect on top of the glass
                    boxShadow: active
