...
interface StarButtonProps extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, 'children'> {
  children: ReactNode
  lightWidth?: number
  duration?: number
  lightColor?: string
  ...
  borderWidth?: number
  /** Seconds for one full color-cycle revolution. Set to 0 to disable color cycling. */
  hueCycleDuration?: number
  className?: string
}
        ...
        style={
          {
            ...
            //   star-btn  → moves the streak around the perimeter
            //   hue-cycle → shifts its colour through the full spectrum
            // Chained inline because Tailwind's animate-* utility only
            // accepts one animation at a time.
            animation: hueCycleDuration > 0
              ? `star-btn ${duration}s linear infinite, hue-cycle ${hueCycleDuration}s linear infinite`
              : `star-btn ${duration}s linear infinite`,
          } as CSSProperties
        }
      />
      <div
