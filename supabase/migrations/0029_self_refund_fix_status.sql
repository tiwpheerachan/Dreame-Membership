-- ============================================================
-- Migration 0029: Fix self_refund_coupon — allow 'confirmed' status
--
-- ปัญหา: redeem_reward + auto-issue code ตั้ง status='confirmed' (ไม่ใช่ 'redeemed')
-- → user ที่ลอง refund coupon ปกติได้ error "ไม่สามารถแลกคืนได้ (status: confirmed)"
--
-- แก้: ขยาย allowed status ของ self_refund_coupon เป็น:
--   ('pending', 'redeemed', 'confirmed')
-- ห้าม: shipping/delivered/cancelled/expired (ของส่งหรือใช้ไปแล้ว)
-- ============================================================
...
BEGIN
  ...
  -- ── FIX: รวม 'confirmed' (state ปกติของ code ที่ออกแล้วยังไม่ใช้) ──
